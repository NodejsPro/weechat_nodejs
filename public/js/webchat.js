...
var window_webchat_hash = 'webchat';
...
function embot_init(url, connect_page_id_param, preview_flg_param, refresh_log_flg_param, force_log_param) {
            ...
            if(event.data != void 0) {
                ...
                if(event.data.connect_page_id != void 0 && event.data.connect_page_id == g_connect_page_id) {
                    ...
                    if(event.data.iframe_setting != void 0) {
                        ...
                        createDispatchEvent('webchat_load_ready');
                        ...
                        //clear all cookie
                        clearAllCookie();
                    } else if(event.data.header_height){
                        iframe.setAttribute("data-header_height", event.data.header_height);
                        var ifm_height_attr = iframe.getAttribute('data-height');
                        webchat_ctn.style.bottom = '-' + (ifm_height_attr - event.data.header_height + 1) + 'px';
                    }
                }
            }
    ...
    window.addEventListener("popstate", function(e){
        if(isMobile()) {
            if(window.location.hash != ('#' + window_webchat_hash)) {
...
function chatBoxOpenClose(create_cookie_flg) {
    if(webchat_ctn.getAttribute('status') == 'opening'){
        ...
        if(isMobile() && window.location.hash == ('#' + window_webchat_hash)) {
            history.back();
        }
...
function minimizeWc(create_cookie_flg) {
    if(webchat_ctn.getAttribute('status') == 'opening') {
        ...
        if (isMobile()) {
            ...
            document.documentElement.classList.remove('webchat-lock-screen');
...
function maximizeWc(create_cookie_flg) {
    if(webchat_ctn.getAttribute('status') == 'closed') {
        ...
        if (isMobile()) {
            ...
            history.pushState("efo", null, "#" + window_webchat_hash);

            //add class to lock client page. Delay with animation open chatbox
            setTimeout(function () {
                document.documentElement.classList.add('webchat-lock-screen');
            }, 500);
        } else {
...
function btag (event, action_name) {
    /*var params = {
    ...
    // src="origin_url?event=event&action_name=open_close&...

    img_elm.onload = img_elm.onerror = function () {
        img_elm.onload = null;
        img_elm.onerror = null;
